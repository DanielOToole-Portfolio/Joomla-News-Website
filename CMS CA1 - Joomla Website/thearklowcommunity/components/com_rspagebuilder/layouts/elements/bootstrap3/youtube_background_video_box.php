...
if (!empty($element_options['video_api_key']) && !empty($element_options['video_url'])) {
	
	// YouTube Background Video Box video settings
	$video_settings = '{apiKey:\'' . 'AIzaSyB_z8WrsKw2kIplh5kBc6xTawEDu91V5dQ' . '\',';
...
if ((!empty($element_options['video_api_key']) && !empty($element_options['video_url'])) && ((!empty($element_options['title']) && !empty($title_show)) || !empty($element_options['content']))) {
?>
...
<div id="<?php echo $id; ?>" class="<?php echo $class; ?>"<?php echo RSPageBuilderHelper::buildStyle($style); ?>>
	<div class="rspbld-content-container">
		...
		<?php if (!empty($element_options['video_api_key']) && !empty($element_options['video_url'])) { ?>
			<div class="rspbld-youtube-player" data-property="<?php echo $video_settings; ?>"></div>
		<?php } ?>
	</div>
</div>
<?php } ?>