...
if (!empty($element_options['map_api_key'])) {
	
	// Load Google Map script
	$doc->addScript('http'.(JURI::getInstance()->isSSL() ? 's' : '').'://maps.google.com/maps/api/js?key='.$element_options['map_api_key']);
	...
	if (count($markers)) {
		foreach ($markers as $marker_index => $marker) {
			...
			if (!empty($marker_options['marker_address'])) {
				...
				if (!empty($marker_options['marker_title']) || !empty($marker_options['marker_content'])) {
					if (!empty($marker_options['marker_title'])) {
					...
					$map_script .= '	var infowindow = new google.maps.InfoWindow({
											content: ' . json_encode($infowindow_content) . ',
											maxWidth: 250
										});';
										
					$map_script .= '	if (marker && infowindow) {
				...
				$map_script .= '	var marker = new google.maps.Marker({
										position: {
