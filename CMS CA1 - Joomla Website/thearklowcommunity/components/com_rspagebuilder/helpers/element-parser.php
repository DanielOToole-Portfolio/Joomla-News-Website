...
class ElementParser {
    ...
    public static function viewPage ($content, $bootstrap_version, $full_width, $animate) {
		if (is_array($content)) {
				...
				foreach ($row['columns'] as $column) {
					...
					if ($animate) {
						if (!empty($column_options['animation_type'])) {
							...
							RSPageBuilderHelper::loadAsset('component', 'scroll.animations.min.js');
						}
					...
					foreach ($column['elements'] as $element) {
						$html .= RSPageBuilderHelper::loadElementLayout($element, $bootstrap_version);
