		...
		<div class="columns<?php echo (($row->options->row_full_width == '0') ? ' container' : ''); ?>">
			...
			<div class="column span<?php echo $column->grid; ?>">
				...
				foreach ($column->elements as $key => $element) {
					if ($element->type == 'rspbld_google_map') {
						$this->map_api_key = $element->options->map_api_key;
					}
				...
				?>
					<div class="element-container">
						<div class="element">
							...
							<input class="element-json" type="hidden" value="<?php echo str_replace('"', '&quot;', json_encode(RSPageBuilderHelper::toArray($element))); ?>">
						</div>
					</div>
			<?php
			}
			...
			JFactory::getDocument()->addScript('http://maps.google.com/maps/api/js?key='.$this->map_api_key);
			?>
					...
					<input class="column-json" type="hidden" value="<?php echo str_replace('"', '&quot;', json_encode($json_column)); ?>">
				</div>
		<?php } ?>
			</div>
			<div class="row-options hidden">
			</div>
			<input class="row-json" type="hidden" value="<?php echo str_replace('"', '&quot;', json_encode($json_row)); ?>">
		</div>
	</div>
	<?php } ?>
</div>
		...
		<div class="row">
			<div class="columns">
				<div class="column span12">
					...
					<input class="column-json" type="hidden" value="<?php echo str_replace('"', '&quot;', json_encode($this->column_config)); ?>">
				</div>
			</div>
			<div class="row-options hidden">
			</div>
			<input class="row-json" type="hidden" value="<?php echo str_replace('"', '&quot;', json_encode($this->row_config)); ?>">
		</div>
	</div>
</div>
<?php } ?>
